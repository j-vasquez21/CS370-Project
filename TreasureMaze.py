...
class TreasureMaze(object):
    ...
    def update_state(self, action):
        nrows, ncols = self.maze.shape
        pirate_row, pirate_col, mode = self.state # fixed syntax error
        ...
        if self.maze[pirate_row, pirate_col] > 0.0:
            ...
            self.maze[pirate_row, pirate_col] = visited_mark # ADDED 
        ...
        self.maze[nrow, ncol] = pirate_mark # ADDED
    ...
    def draw_env(self):
        canvas = np.copy(self.maze)
        nrows, ncols = self.maze.shape
        # Instead of clearing all visual marks, keep visited cells visible
        for r in range(nrows):
            for c in range(ncols):
                if self._maze[r, c] == 0.0:
                    canvas[r, c] = 0.0  # keep walls black
                elif (r, c) in self.visited:
                    canvas[r, c] = visited_mark
                else:
                    canvas[r, c] = 1.0  # unvisited free cell white
        # draw pirate
